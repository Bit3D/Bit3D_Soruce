...
void DXAPP::DxDebugHelper::CheckForHRESULTError(HRESULT hr,LPCWSTR Caption,LPCWSTR ErrorMsg)
{
	if(FAILED(hr))
		MessageBox(_hwnd,ErrorMsg,Caption,0);
}

DXAPP::DxDebugHelper::DxDebugHelper()
{
	SecureZeroMemory(&_hwnd,sizeof(_hwnd));
}

void DXAPP::DxDebugHelper::StartBenchmark(string BenchmarkName)
{
	BegineClock = clock();
...
void DXAPP::DxDebugHelper::StopBenchmark()
{
	EndClock = clock();

	double timeInSec = double(EndClock - BegineClock)/ CLOCKS_PER_SEC;
	double timeInMilleSec = timeInSec/1000;
	char buffer[123],*Header;

	strcpy(buffer,EntryName.c_str());
	ofstream ot;
	ifstream ip;
	...
	if(!ip)
		...
		Header = "";
	ot.open("Debug.txt",fstream::out | fstream::app);

	if(LastEntry != EntryName)
	{
	...
	ot << buffer << " : ";
	ot << timeInMilleSec << " ms" <<  " | " << timeInSec << " sec"<< endl;
...
void DXAPP::DxDebugHelper::DisplayResults()
{
//system("\"C:\\WINDOWS\\system32\\notepad.exe\" Debug.txt");
...
PROCESS_INFORMATION pi = {0};
	if(!CreateProcess(L"C:\\WINDOWS\\system32\\notepad.exe"
		...
		NULL,
		NULL,
		L"C:\\Users\\Karthik\\Documents\\Visual Studio 2012\\Projects\\Win32Project1\\Win32Project1\\"
		,&startup_info,&pi
		
		))
		CheckForHRESULTError(S_FALSE,L"Error",L"Error launching notepad.exe");
...
DXAPP::DXAPP()
{
	...
	SecureZeroMemory(&DebugHelper,sizeof(DebugHelper));

}

void DXAPP::CreateD3DDevice(HWND hwnd,RECT rect)
{
	DebugHelper._hwnd = hwnd;
	...
	HR( d3dDevice->CreateRenderTargetView(backbuffer,NULL,&RenderTarget),
		L"Couldnot Create RenderTexture" ,L"ERROR"
		);
...
void DXAPP::HR(HRESULT hr,LPCWSTR ErrorText,LPCWSTR Caption)
{
	DebugHelper.CheckForHRESULTError(hr,ErrorText,Caption);
}
